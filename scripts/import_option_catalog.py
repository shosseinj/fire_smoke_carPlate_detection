...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Import previous-project options/endpoints from Excel into the agentic catalog")
    parser.add_argument("--excel", required=True, help="Path to the Excel options catalog")
    parser.add_argument("--sheet", help="Optional worksheet name")
    parser.add_argument(
        "--output",
        default=".agentic/migration/legacy-endpoint-catalog.json",
        help="Catalog JSON output path",
    )
    parser.add_argument(
        "--source-copy",
        default=".agentic/migration/source/previous-project-options.xlsx",
        help="Where to keep a copy of the imported Excel file",
    )
    parser.add_argument("--replace-progress", action="store_true", help="Do not preserve progress for matching item IDs")
    args = parser.parse_args()
    try:
        result = build_catalog(
            Path(args.excel).expanduser().resolve(),
            Path(args.output).expanduser().resolve(),
            Path(args.source_copy).expanduser().resolve(),
            requested_sheet=args.sheet,
            preserve_existing=not args.replace_progress,
        )
    except Exception as exc:
        print(f"ERROR: {exc}", file=sys.stderr)
        raise SystemExit(2)
    print(
        f"Imported {result['source']['item_count']} catalog items "
        f"from {result['source']['original_filename']} ({result['source']['sheet']})."
    )
